...
long ClientDataCommunication::UnInit()
{
	TSDEBUG(L"Enter, m_dwEventTotal = %u", m_dwEventTotal);

	for (DWORD dwIndex = 0; dwIndex < m_dwEventTotal; dwIndex ++)
	{
		FreeSocketInformation(dwIndex);
	}
...
long ClientDataCommunication::PostConnectCompleteMsg(BOOL bRet)
{
	...
	if (!::PostMessage(m_hIpcWnd, WM_DATACOMMUNICATION_LINKCALLBACK, (WPARAM)lpConnectCallbackData, NULL))
	{
		DWORD dwErrCode = ::GetLastError();
...
long ClientDataCommunication::PostSendCompleteMsg(DWORD dwId, DWORD dwErrCode)
{
	...
	if (!::PostMessage(m_hIpcWnd, WM_DATACOMMUNICATION_SENDCALLBACK, (WPARAM)lpSendCallbackData, NULL))
	{
		DWORD dwErrCode = ::GetLastError();
...
long ClientDataCommunication::PostRecvCompleteMsg(const char* szData, DWORD dwNumberOfBytesRecv)
{
	...
	if (!::PostMessage(m_hIpcWnd, WM_DATACOMMUNICATION_RECVCALLBACK, (WPARAM)lpRecvCallbackData, NULL))
	{
		DWORD dwErrCode = ::GetLastError();
...
long ClientDataCommunication::PostDisconnectMsg()
{
	...
	if (!::PostMessage(m_hIpcWnd, WM_DATACOMMUNICATION_DISCONNECTCALLBACK, NULL, NULL))
	{
		DWORD dwErrCode = ::GetLastError();
