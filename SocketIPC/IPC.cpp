...
{
	m_LinkCallbackInfo.pfnOnLinkCallback = NULL;
	m_LinkCallbackInfo.pUserData = NULL;
	m_RecvCallbackInfo.pfnRecvCallback = NULL;
	m_RecvCallbackInfo.pUserData = NULL;
	m_DisconnectCallbackInfo.pfnDisconnectCallback = NULL;
	m_DisconnectCallbackInfo.pUserData = NULL;
}
...
LRESULT IPC::OnSendCallback(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	{
		it->second.pfnOnSendCallback(dwErrCode, dwId, it->second.pUserData);
...
LRESULT IPC::OnDisconnectCallback(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (m_DisconnectCallbackInfo.pfnDisconnectCallback)
	{
		TSDEBUG(L"Call pfnDisconnectCallback");
...
long IPC::Init(IpcType ipcType)
{
	...
	if (!IsWindow())
	{
		TSDEBUG(L"Create Window");
		m_hWnd = Create(HWND_MESSAGE, NULL, NULL);
		if (NULL == m_hWnd)
		{
...
long IPC::UnInit()
{
	...
	return m_DataCommunicationInterface->UnInit();
}
...
long IPC::Send(DWORD dwId, LPSOCKETIPC_STREAM lpSocketIpcStream, ONSENDCALLBACK pfnOnSendCallback, void* pUserData)
{
	...
	long lLen = CommonTool::CombineSocketIpcData(0, dwId, lpSocketIpcStream, NULL);
	TSDEBUG(L"lLen = %d", lLen);

	char* szData = new char[lLen];
	memset(szData, 0, lLen);
	long lRet = CommonTool::CombineSocketIpcData(0, dwId, lpSocketIpcStream, szData);
	if (0 != lRet)
	{
	...
	lRet = m_DataCommunicationInterface->SendData(dwId, szData, lLen);
